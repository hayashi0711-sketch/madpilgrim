alter table public.spots add column if not exists title_en text;
alter table public.spots add column if not exists spot_name_en text;
alter table public.spots add column if not exists scene_timestamp_en varchar(80);
...
create or replace function public.upsert_spot_candidate_v2(
  p_slug text,
  p_title text,
  p_category text,
  p_spot_name text,
